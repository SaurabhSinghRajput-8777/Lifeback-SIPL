...
export function RoleSelectionModal({ isOpen, onClose }: RoleSelectionModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div 
        className="relative w-full max-w-2xl bg-background border border-border dark:border-white/10 rounded-2xl shadow-[0_40px_80px_rgba(0,0,0,0.1)] dark:shadow-[0_40px_80px_rgba(0,0,0,0.8)] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-500"
        role="dialog"
        aria-modal="true"
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-muted-foreground hover:text-foreground bg-muted/50 hover:bg-muted rounded-full transition-colors cursor-pointer"
          aria-label="Close modal"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="p-8 sm:p-10 text-center border-b border-border dark:border-white/10">
          ...
          <h2 id="modal-title" className="text-[28px] sm:text-[32px] font-heading font-bold text-foreground mb-3 tracking-tight">
            Create Your LifeBack Account
          </h2>
          <p className="text-[16px] font-body text-muted-foreground">
            Choose the experience that best describes you.
          </p>
        </div>

        <div className="p-8 sm:p-10 grid sm:grid-cols-2 gap-6 bg-muted/20 dark:bg-[#18181b]">
          {/* USER CARD */}
          <button
            onClick={() => handleRoleSelect(ROLES.USER)}
            className={`group relative text-left p-6 rounded-xl border transition-all duration-300 overflow-hidden cursor-pointer ${
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <h3 className="text-[20px] font-heading font-bold text-foreground mb-2">User</h3>
              <p className="text-[14px] font-body text-muted-foreground leading-relaxed flex-grow">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <h3 className="text-[20px] font-heading font-bold text-foreground mb-2">Clinician</h3>
              <p className="text-[14px] font-body text-muted-foreground leading-relaxed flex-grow">
